...
void clif_guild_basicinfo(struct map_session_data *sd) {
	...
	if(bg != NULL) {
		...
		if((bg_sd = bg->members[0].sd) == NULL)
			WFIFOL(fd,70+offset+20) = 0;
		else
			WFIFOL(fd,70+offset+20) = bg_sd->status.char_id;  // leader
	} else
...
void clif_parse_EquipItem(int fd,struct map_session_data *sd)
{
	...
	if (sd->state.refineui_open) {
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], "[Refinador] Proibido equipar enquanto estiver refinando.", false, SELF);
		return;
	}
